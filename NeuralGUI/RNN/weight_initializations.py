...
lr = 0.005 #The learning rate for the RBM

def main(songList):
	#Load the Songs
	songList = songList.split(',')
	songs = midi_manipulation.get_songs(songList)
	...
	with tf.Session() as sess:
		...
		directory = (sys.path[0] + '/parameter_checkpoints')
		if not os.path.exists(directory):
			os.makedirs(directory)

		#Save the initialized model here
		save_location = (sys.path[0] + '/parameter_checkpoints/initialized.ckpt')
